<template>
  <li class="vue-tree-item">
    <div class="item-wrapper" onselectstart="return false;" :class="{active: currentId == self.id}">
      <span class="menu iconfont icon-xiala1" v-if="options.isDropDownMenu"
      ...
      <span
        ...
        :class="isBold"
        @click="handle($event)"
        @contextmenu.prevent="handleMenu($event)"
      >
       <i  v-if="!isFolder" :class="[this.options.leafIcon]"></i> <span @click="toggle" >{{ model[options.label] }}</span>
      </span>
    </div>
    <ul v-if="isFolder" v-show="open" class="vue-tree-list">
      <tree-item
        ...
        :key="idx"
        @handle="emitHandle"
        @handle-menu="emitHandleMenu"
        @handle-icon="emitHandleIcon"
...
<script>
  export default {
    ...
    computed: {
      ...
      self () {
        let self = Object.assign({}, this.model, {children: [],isFolder:this.isFolder})
    ...
    methods: {
      ...
      handle (e) {
        this.currentId = this.self.id  // currentId 应该存在store里面
        let self = Object.assign({}, this.self, {e:e})
        this.emitHandle(self)
      },
      ...
      handleIcon(e){
        ...
        if(!this.options.isDropDownMenu) return 
        let self = Object.assign({}, this.self, {e:e})
        
        this.emitHandleIcon(self)
      },
      emitHandleIcon(item){
